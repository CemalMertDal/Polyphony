#!/usr/bin/env python3
"""agy-migrate.py — migrate a Claude Code setup to Antigravity CLI (agy).

Read-only by default: prints a per-unit plan and exits. `--apply` performs it.

Design constraints, all of them learned the hard way from probing agy 1.1.12
(see docs/MIGRATION.md for the full findings):

  * The Claude Code config dir is treated as READ-ONLY. The only file this tool
    can ever create on the Claude side is an `AGENTS.md` symlink beside an
    existing `CLAUDE.md`, and only under --include-repos.
  * Antigravity rules are silently ignored unless their frontmatter carries
    `trigger: always_on`. No error, no warning — they just never load. So
    migrated memory is REWRITTEN with that frontmatter, never stripped.
  * Workspace customizations (`.agents/`) only load when the session is bound
    to an agy project. Writing `.agents/rules/` without registering the repo in
    ~/.gemini/config/projects/ produces rules that never fire.
  * `~/` is NOT expanded in the global skills.json, so entry paths are absolute.
  * `agy plugin import claude` only scans ~/.claude/plugins/<name>/ one level
    deep, which no modern Claude Code install matches — and it does not follow
    symlinks. It needs no auth, though, so we COPY the real plugins flat into a
    staging HOME, run the native importer there, and repair its output.
"""

import argparse
import json
import os
import re
import shutil
import subprocess
import sys
import tempfile
import time
import uuid
from datetime import datetime, timezone

# --- constants ---------------------------------------------------------------

UNITS = ["plugins", "skills", "claudemd", "memory", "mcp", "settings"]

# Antigravity refuses to load a rule or workflow file larger than this.
RULE_MAX_CHARS = 12000

MARKER = "agy-migrate"
MARKER_MD = f"<!-- generated by {MARKER}; delete this line to protect your edits -->"

MEMORY_PLUGIN = "claude-code-memory"

# Claude tool name -> Antigravity step-type-derived tool name, for hook matchers.
# Antigravity derives tool names by lowercasing CORTEX_STEP_TYPE_* and dropping the
# prefix, so these are exact. Anything absent here is reported, not guessed.
TOOL_MAP = {
    "Bash": "run_command",
    "Read": "view_file",
    "Grep": "grep_search",
    "Glob": "find",
    "LS": "list_directory",
    "WebFetch": "read_url_content",
    "WebSearch": "search_web",
    "Task": "invoke_subagent",
    "Agent": "invoke_subagent",
    "NotebookEdit": "edit_notebook",
}
# Claude's Write/Edit have no single counterpart; agy proposes edits via
# propose_code and writes blobs via write_blob. Match both and say so.
TOOL_MAP_AMBIGUOUS = {"Write": "propose_code|write_blob", "Edit": "propose_code|write_blob"}

# PreToolUse/PostToolUse take {matcher, hooks:[…]}; PreInvocation/PostInvocation/Stop
# take a flat list of handlers. Claude's other five events have no counterpart.
HOOK_EVENTS_GROUPED = {"PreToolUse", "PostToolUse"}
HOOK_EVENT_MAP = {
    "PreToolUse": "PreToolUse",
    "PostToolUse": "PostToolUse",
    "UserPromptSubmit": "PreInvocation",
    "Stop": "Stop",
}

C = {
    "ok": "\033[32m", "warn": "\033[33m", "skip": "\033[90m",
    "err": "\033[31m", "hdr": "\033[1m", "off": "\033[0m",
}
if not sys.stdout.isatty() or os.environ.get("NO_COLOR"):
    C = {k: "" for k in C}


# --- paths -------------------------------------------------------------------

def home():
    return os.path.expanduser("~")


def claude_dir():
    """Honour CLAUDE_CONFIG_DIR — this user runs a second profile at ~/.claude-pro."""
    return os.path.abspath(os.path.expanduser(
        os.environ.get("CLAUDE_CONFIG_DIR") or os.path.join(home(), ".claude")))


def claude_json_path():
    """.claude.json sits INSIDE a relocated config dir, but beside the default one."""
    inside = os.path.join(claude_dir(), ".claude.json")
    if os.path.isfile(inside):
        return inside
    return os.path.join(os.path.dirname(claude_dir()), ".claude.json")


def gemini_root():
    return os.path.join(home(), ".gemini")


def gemini_config():
    return os.path.join(gemini_root(), "config")


def agy_settings_path():
    return os.path.join(gemini_root(), "antigravity-cli", "settings.json")


def state_dir():
    """Our own namespace, so nothing we track pollutes the shared config dir."""
    return os.path.join(gemini_root(), ".agy-migrate")


def manifest_path():
    return os.path.join(state_dir(), "manifest.json")


# --- small io helpers --------------------------------------------------------

def read_json(path, default=None):
    try:
        with open(path, encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return default


def write_json(path, obj):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    tmp = path + ".tmp"
    with open(tmp, "w", encoding="utf-8", newline="\n") as f:
        json.dump(obj, f, indent=2, ensure_ascii=False)
        f.write("\n")
    os.replace(tmp, path)


def read_text(path):
    try:
        with open(path, encoding="utf-8", errors="replace") as f:
            return f.read()
    except Exception:
        return ""


def now_iso():
    return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%fZ")


def encode_project_dir(path):
    """Claude Code's projects/ dir name. Lossy: separators, '_' and '.' become '-',
    which is why this only ever runs FORWARD, never as a decode."""
    return re.sub(r"[:/\\_.]", "-", path)


def default_roots():
    """Where to hunt for CLAUDE.md and .mcp.json.

    Walking all of $HOME is slow and picks up unrelated trees. Claude Code already
    records every directory it has been run in, as a real absolute path, so use those
    and drop any nested under another so no tree is walked twice.
    """
    cj = read_json(claude_json_path(), None) or {}
    paths = sorted({os.path.abspath(p) for p in (cj.get("projects") or {})
                    if os.path.isdir(p)})
    roots = [p for p in paths
             if not any(p != q and p.startswith(q.rstrip("/") + os.sep) for q in paths)]
    return roots or [home()]


def git_root(path):
    try:
        out = subprocess.run(["git", "-C", path, "rev-parse", "--show-toplevel"],
                             capture_output=True, text=True, timeout=10)
        if out.returncode == 0:
            return out.stdout.strip()
    except Exception:
        pass
    return None


# --- plan model --------------------------------------------------------------

class Plan:
    """Collects every intended change so --apply and dry-run share one code path."""

    def __init__(self):
        self.items = []

    def add(self, unit, level, action, target, detail="", fn=None):
        self.items.append({"unit": unit, "level": level, "action": action,
                           "target": target, "detail": detail, "fn": fn})

    def by_unit(self, unit):
        return [i for i in self.items if i["unit"] == unit]

    def counts(self):
        c = {}
        for i in self.items:
            c[i["level"]] = c.get(i["level"], 0) + 1
        return c


class Manifest:
    """Records exactly what we created, so --uninstall can undo precisely."""

    def __init__(self):
        d = read_json(manifest_path(), None) or {}
        self.files = d.get("files", [])
        self.symlinks = d.get("symlinks", [])
        self.dirs = d.get("dirs", [])       # ensured to exist; rmdir only if empty
        self.trees = d.get("trees", [])     # created wholesale by us; rmtree on undo
        # Shared config files we happened to create. agy and the desktop apps append
        # to these after a migration, so undo removes our KEYS and only then deletes
        # the file if nothing else ended up in it.
        self.created_json = d.get("created_json", [])
        self.json_keys = d.get("json_keys", {})   # file -> {"pointer": [keys]}
        self.projects = d.get("projects", [])
        self.runs = d.get("runs", [])

    def save(self):
        write_json(manifest_path(), {
            "files": sorted(set(self.files)),
            "symlinks": sorted(set(self.symlinks)),
            "dirs": sorted(set(self.dirs)),
            "trees": sorted(set(self.trees)),
            "created_json": sorted(set(self.created_json)),
            "json_keys": self.json_keys,
            "projects": sorted(set(self.projects)),
            "runs": self.runs[-20:],
        })

    def note_key(self, path, pointer, key):
        self.json_keys.setdefault(path, {}).setdefault(pointer, [])
        if key not in self.json_keys[path][pointer]:
            self.json_keys[path][pointer].append(key)


def backup(paths):
    """Timestamped copy of every Antigravity file we are about to touch."""
    stamp = time.strftime("%Y%m%d-%H%M%S")
    dest = os.path.join(state_dir(), "backups", stamp)
    for p in paths:
        if not os.path.exists(p):
            continue
        rel = os.path.relpath(p, gemini_root())
        out = os.path.join(dest, rel)
        os.makedirs(os.path.dirname(out), exist_ok=True)
        if os.path.isdir(p):
            shutil.copytree(p, out, symlinks=True, dirs_exist_ok=True)
        else:
            shutil.copy2(p, out)
    return dest


def generated(path):
    """True when we may overwrite: the file carries our marker (or is absent).
    A hand-written file without the marker is never clobbered."""
    if not os.path.exists(path):
        return True
    return MARKER in read_text(path)


# --- frontmatter -------------------------------------------------------------

FM_RE = re.compile(r"\A---\r?\n(.*?)\r?\n---\r?\n?", re.S)


def split_frontmatter(text):
    m = FM_RE.match(text)
    if not m:
        return {}, text
    raw, body = m.group(1), text[m.end():]
    meta, key, buf = {}, None, []
    for line in raw.split("\n"):
        if re.match(r"^[A-Za-z0-9_.-]+\s*:", line) and not line.startswith(" "):
            if key:
                meta[key] = "\n".join(buf).strip()
            key, _, rest = line.partition(":")
            key = key.strip()
            buf = [rest.strip()]
        elif key:
            buf.append(line.strip())
    if key:
        meta[key] = "\n".join(buf).strip()
    for k, v in list(meta.items()):
        if v.startswith((">-", ">", "|-", "|")):
            meta[k] = v.lstrip(">|-").strip()
        meta[k] = meta[k].strip().strip('"').strip("'")
    return meta, body


def resolve_wikilinks(body, siblings):
    """[[slug]] -> [slug](slug.md) when the target exists, else plain text."""
    def sub(m):
        slug = m.group(1).strip()
        return f"[{slug}]({slug}.md)" if slug in siblings else slug
    return re.sub(r"\[\[([^\]]+)\]\]", sub, body)


def chunk_body(body, budget):
    """Split on blank lines so a chunk never cuts mid-paragraph."""
    if len(body) <= budget:
        return [body]
    parts, cur = [], ""
    for para in body.split("\n\n"):
        candidate = (cur + "\n\n" + para) if cur else para
        if len(candidate) > budget and cur:
            parts.append(cur)
            cur = para
        else:
            cur = candidate
        # A single paragraph over budget still has to be cut somewhere.
        while len(cur) > budget:
            parts.append(cur[:budget])
            cur = cur[budget:]
    if cur:
        parts.append(cur)
    return parts


def render_rule(name, description, body, part=None, total=None):
    desc = description or f"Migrated Claude Code memory: {name}"
    if total and total > 1:
        desc = f"{desc} (part {part}/{total})"
    desc = desc.replace("\n", " ").strip()
    return (
        "---\n"
        "trigger: always_on\n"
        f"description: {json.dumps(desc, ensure_ascii=False)}\n"
        "---\n"
        f"{MARKER_MD}\n"
        f"# {name}\n\n"
        f"{body.strip()}\n"
    )


# --- unit: skills ------------------------------------------------------------

def unit_skills(plan, mf):
    src = os.path.join(claude_dir(), "skills")
    if not os.path.isdir(src):
        plan.add("skills", "skip", "no-op", src, "no user skills on the Claude side")
        return
    names = sorted(d for d in os.listdir(src)
                   if os.path.isfile(os.path.join(src, d, "SKILL.md")))
    if not names:
        plan.add("skills", "skip", "no-op", src, "no directory contains a SKILL.md")
        return

    # Report Claude-only frontmatter that agy will silently ignore.
    for n in names:
        meta, _ = split_frontmatter(read_text(os.path.join(src, n, "SKILL.md")))
        dead = [k for k in ("allowed-tools", "model", "disable-model-invocation")
                if k in meta]
        if dead:
            plan.add("skills", "warn", "ignored-frontmatter", n,
                     f"{', '.join(dead)} has no effect in Antigravity")

    cfg = os.path.join(gemini_config(), "skills.json")
    existing = read_json(cfg, None) or {}
    entries = existing.get("entries") or []
    if any(os.path.abspath(os.path.expanduser(e.get("path", ""))) == src for e in entries):
        plan.add("skills", "skip", "already-registered", cfg,
                 f"{len(names)} skill(s) already registered")
        return

    preexisting = os.path.exists(cfg)

    def fn():
        cur = read_json(cfg, None) or {}
        ents = cur.get("entries") or []
        ents.append({"path": src})          # absolute: '~' is not expanded here
        cur["entries"] = ents
        write_json(cfg, cur)
        # Always per-entry: skills.json is shared, so even when we created it, agy may
        # have added entries of its own by the time anyone runs --uninstall.
        mf.note_key(cfg, "entries", src)
        if not preexisting:
            mf.created_json.append(cfg)

    plan.add("skills", "ok", "register", cfg,
             f"read {len(names)} skill(s) in place (entries += {src})", fn)


# --- unit: CLAUDE.md ---------------------------------------------------------

SKIP_DIRS = {"node_modules", ".git", ".venv", "venv", "dist", "build", "__pycache__"}


def excluded_roots():
    """Never scan either tool's own config tree.

    `~/.claude/plugins/marketplaces/` holds cloned marketplace catalogues — hundreds
    of third-party `.mcp.json` files the user never configured. A real run over `$HOME`
    pulled 40 servers out of one. Plugin-owned MCP is the plugins unit's job anyway.
    """
    return (claude_dir(), gemini_root(), state_dir(),
            os.path.join(home(), "Library"))


def under_excluded(path):
    """True only when `path` is the excluded root itself or genuinely inside it.

    A bare startswith() would prune `~/.claude-pro` (a second Claude Code profile,
    selected via CLAUDE_CONFIG_DIR) because it shares a prefix with `~/.claude`, and
    `~/Library-notes` because of `~/Library`. The exclusion is silent, so that would
    just look like the tool ignoring a directory for no reason.
    """
    p = os.path.abspath(path)
    return any(p == e or p.startswith(e.rstrip(os.sep) + os.sep)
               for e in (os.path.abspath(x) for x in excluded_roots()))


def walk_user_tree(root):
    """os.walk with vendor dirs and both config trees pruned."""
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [
            d for d in dirnames
            if d not in SKIP_DIRS
            and not d.startswith(".")
            and not under_excluded(os.path.join(dirpath, d))
        ]
        yield dirpath, dirnames, filenames


def find_claude_mds(roots):
    out = []
    for root in roots:
        for dirpath, _dirnames, filenames in walk_user_tree(root):
            if "CLAUDE.md" in filenames:
                out.append(os.path.join(dirpath, "CLAUDE.md"))
    return sorted(out)


def unit_claudemd(plan, mf, roots, include_repos):
    mds = find_claude_mds(roots)
    if not mds:
        plan.add("claudemd", "skip", "no-op", ", ".join(roots), "no CLAUDE.md found")
        return
    if not include_repos:
        plan.add("claudemd", "skip", "needs-flag", f"{len(mds)} file(s)",
                 "pass --include-repos to create AGENTS.md symlinks")
        return
    for md in mds:
        agents = os.path.join(os.path.dirname(md), "AGENTS.md")
        if os.path.islink(agents):
            plan.add("claudemd", "skip", "exists", agents, "already a symlink")
            continue
        if os.path.exists(agents):
            plan.add("claudemd", "warn", "conflict", agents,
                     "a real AGENTS.md already exists; not overwriting")
            continue

        def fn(agents=agents, md=md):
            try:
                os.symlink("CLAUDE.md", agents)
            except OSError:
                if os.name != "nt":
                    raise
                # Windows commonly disables symlink creation outside Developer
                # Mode. A same-volume hard link preserves the shared contents
                # without requiring elevation.
                os.link(md, agents)
            mf.symlinks.append(agents)

        plan.add("claudemd", "ok", "symlink", agents, "-> CLAUDE.md", fn)


# --- unit: memory ------------------------------------------------------------

def memory_sources():
    """Every projects/<enc>/memory/ dir, resolved back to a real cwd where possible.

    Resolution is forward-only: we re-encode each known project path from
    ~/.claude.json and compare. Decoding the directory name is impossible because
    '/', '_' and '.' all collapse to '-'.
    """
    base = os.path.join(claude_dir(), "projects")
    if not os.path.isdir(base):
        return []
    cj = read_json(claude_json_path(), None) or {}
    # The encoding is lossy, so two distinct project paths can produce the same
    # directory name. Picking one by dict order could file a repo's memory into a
    # different repo's .agents/rules/, so a collision resolves to nothing and the
    # dir is reported as unresolved instead.
    by_enc = {}
    for path in (cj.get("projects") or {}):
        enc = encode_project_dir(path)
        by_enc[enc] = None if enc in by_enc else path

    out = []
    for enc in sorted(os.listdir(base)):
        mem = os.path.join(base, enc, "memory")
        if not os.path.isdir(mem):
            continue
        files = sorted(f for f in os.listdir(mem)
                       if f.endswith(".md") and f != "MEMORY.md")
        if not files:
            continue
        cwd = by_enc.get(enc)
        if cwd and not os.path.isdir(cwd):
            cwd = None
        out.append({"enc": enc, "dir": mem, "files": files, "cwd": cwd})
    return out


def rules_from_memory_dir(src):
    """Turn one memory dir into (filename, content) rule pairs."""
    slugs = {f[:-3] for f in src["files"]}
    rules = []
    for fname in src["files"]:
        text = read_text(os.path.join(src["dir"], fname))
        meta, body = split_frontmatter(text)
        name = meta.get("name") or fname[:-3]
        body = resolve_wikilinks(body, slugs)
        # The overhead depends on the part suffix, whose width depends on the total,
        # which depends on the budget. Estimate, then re-measure until it settles.
        total, chunks = 1, [body]
        for _ in range(4):
            overhead = len(render_rule(name, meta.get("description", ""), "",
                                       total, total))
            chunks = chunk_body(body, max(1000, RULE_MAX_CHARS - overhead))
            if len(chunks) == total:
                break
            total = len(chunks)
        total = len(chunks)
        for i, chunk in enumerate(chunks, 1):
            out_name = fname if total == 1 else f"{fname[:-3]}-{i}.md"
            rules.append((out_name,
                          render_rule(name, meta.get("description", ""),
                                      chunk, i, total)))
    return rules


def unit_memory(plan, mf, include_orphans, include_repos, register_projects):
    srcs = memory_sources()
    if not srcs:
        plan.add("memory", "skip", "no-op", os.path.join(claude_dir(), "projects"),
                 "no memory found")
        return

    home_enc = encode_project_dir(home())
    for src in srcs:
        rules = rules_from_memory_dir(src)
        prefix = ""
        split = sum(1 for n, _ in rules if re.search(r"-\d+\.md$", n))

        # The home-dir project is Claude's de-facto global memory; everything else
        # belongs to the repo it came from.
        is_global = src["enc"] == home_enc
        if is_global:
            dest = os.path.join(gemini_config(), "plugins", MEMORY_PLUGIN, "rules")
            scope = "global"
        elif src["cwd"] and not include_repos:
            plan.add("memory", "skip", "needs-flag", src["cwd"],
                     f"{len(src['files'])} note(s); --include-repos writes them to .agents/rules")
            continue
        elif src["cwd"]:
            dest = os.path.join(src["cwd"], ".agents", "rules")
            scope = src["cwd"]
            # Rule discovery walks up only as far as the repository root, so a
            # .agents/ placed in a plain parent directory of several repos is
            # invisible from inside any of them.
            if git_root(src["cwd"]) is None:
                plan.add("memory", "warn", "out-of-reach", src["cwd"],
                         "not a git repository, so nested repos will never see these "
                         "(discovery stops at the repo root) — consider global scope")
        elif include_orphans:
            # Flat, with a filename prefix — NOT a nested rules/<sub>/ directory.
            # Only a flat rules/ is verified to load; a subdirectory would be the
            # same silent no-op this whole tool exists to avoid.
            dest = os.path.join(gemini_config(), "plugins", MEMORY_PLUGIN, "rules")
            prefix = "orphan-" + src["enc"].strip("-") + "-"
            scope = "global (orphan)"
        else:
            plan.add("memory", "warn", "unresolved", src["enc"],
                     f"{len(src['files'])} note(s); source directory no longer exists "
                     "(--include-orphan-memory folds them into global rules)")
            continue

        if prefix:
            rules = [(prefix + n, c) for n, c in rules]
        detail = f"{len(rules)} rule(s) -> {scope}"
        if split:
            detail += f" / {split} split at the 12000-char cap"

        def fn(dest=dest, rules=rules, is_global=is_global, src=src):
            os.makedirs(dest, exist_ok=True)
            mf.dirs.append(dest)
            if is_global or dest.startswith(gemini_config()):
                pj = os.path.join(gemini_config(), "plugins", MEMORY_PLUGIN,
                                  "plugin.json")
                if not os.path.exists(pj):
                    write_json(pj, {"name": MEMORY_PLUGIN,
                                    "description": "Memory migrated from Claude Code"})
                    mf.files.append(pj)
                mf.dirs.append(os.path.dirname(pj))
            for name, content in rules:
                out = os.path.join(dest, name)
                if not generated(out):
                    continue          # user-authored: leave it alone
                with open(out, "w", encoding="utf-8", newline="\n") as f:
                    f.write(content)
                mf.files.append(out)

        plan.add("memory", "ok", "generate", dest, detail, fn)

        if not is_global and src["cwd"] and register_projects:
            plan_project_registration(plan, mf, src["cwd"], "memory")


# --- agy project registration ------------------------------------------------

def existing_project_for(path):
    d = os.path.join(gemini_config(), "projects")
    if not os.path.isdir(d):
        return None
    want = "file://" + path
    for fn in os.listdir(d):
        if not fn.endswith(".json"):
            continue
        p = read_json(os.path.join(d, fn), None) or {}
        for r in (p.get("projectResources") or {}).get("resources") or []:
            if r.get("folderUri") == want or p.get("name") == path:
                return p.get("id")
    return None


def bind_path_to_project(path, pid, mf):
    """Add the cwd -> project id entry each surface keeps in cache/projects.json.

    This is what makes the project selectable for that directory. It is a CACHE, so
    agy may rewrite it — and in print mode it is not consulted at all (see below).
    """
    for surface in ("antigravity-cli", "antigravity", "antigravity-ide"):
        cache = os.path.join(gemini_root(), surface, "cache", "projects.json")
        if not os.path.isfile(cache):
            continue
        d = read_json(cache, None) or {}
        if d.get(path) == pid:
            continue
        d[path] = pid
        write_json(cache, d)
        mf.note_key(cache, "", path)


def plan_project_registration(plan, mf, path, unit):
    """Workspace rules never load unless the session is bound to a project.

    Registration is necessary but NOT sufficient: `agy -p` always uses the id in
    cache/default_project_id.txt (`default-cli-project` out of the box) regardless of
    cwd, so headless runs need `--project <id>` explicitly. Measured on agy 1.1.12 —
    a canary rule fires with `--project`/`--new-project` and stays silent without.
    Saying "registered" and stopping would leave the memory looking migrated and never
    loading, so the id is reported for the user to pass.
    """
    have = existing_project_for(path)
    if have:
        plan.add(unit, "skip", "project-exists", path,
                 f"reusing agy project {have} — run agy there as: "
                 f"agy --project {have}")
        return
    pid = str(uuid.uuid4())

    def fn(pid=pid, path=path):
        out = os.path.join(gemini_config(), "projects", f"{pid}.json")
        write_json(out, {
            "id": pid,
            "name": path,
            "projectResources": {"resources": [{"folderUri": "file://" + path}]},
            "updatedAt": now_iso(),
        })
        mf.files.append(out)
        mf.projects.append(pid)
        bind_path_to_project(path, pid, mf)

    plan.add(unit, "ok", "register-project", path,
             f"register agy project {pid} — headless runs must select it: "
             f"agy --project {pid}", fn)


# --- unit: MCP ---------------------------------------------------------------

UNEXPANDED = re.compile(r"\$\{?CLAUDE_[A-Z_]+\}?")


def translate_mcp(spec):
    """Claude mcpServers entry -> Antigravity. Returns (entry, warnings)."""
    warns = []
    blob = json.dumps(spec)
    if UNEXPANDED.search(blob):
        return None, ["references a Claude Code variable Antigravity never sets "
                      f"({UNEXPANDED.search(blob).group(0)})"]
    url = spec.get("serverUrl") or spec.get("url") or spec.get("httpUrl")
    if url:
        if spec.get("headers"):
            warns.append("headers dropped: unsupported")
        return {"serverUrl": url}, warns
    cmd = spec.get("command")
    if not cmd:
        return None, ["neither command nor url"]
    out = {"command": cmd}
    if spec.get("args"):
        out["args"] = spec["args"]
    if spec.get("env"):
        out["env"] = spec["env"]
    if not shutil.which(cmd) and not os.path.isabs(cmd):
        warns.append(f"`{cmd}` is not on PATH (agy will fail to start it)")
    return out, warns


def collect_mcp(roots):
    """Every place Claude Code keeps the USER's MCP servers, plus the desktop config.

    Vendored plugin catalogues are excluded by construction — see below.
    """
    found = {}   # name -> (spec, source)

    def take(name, spec, source):
        if name not in found:
            found[name] = (spec, source)

    roots_abs = [os.path.abspath(r) for r in roots]

    def in_roots(path):
        p = os.path.abspath(path)
        return any(p == r or p.startswith(r.rstrip(os.sep) + os.sep) for r in roots_abs)

    cj = read_json(claude_json_path(), None) or {}
    for proj, cfg in (cj.get("projects") or {}).items():
        if not in_roots(proj):
            continue
        for name, spec in (cfg.get("mcpServers") or {}).items():
            take(name, spec, f".claude.json:{proj}")
    for name, spec in (cj.get("mcpServers") or {}).items():
        take(name, spec, ".claude.json")

    # Deliberately NOT a filesystem walk. `.mcp.json` sits at a project root by
    # convention, and walking $HOME found vendored catalogues in two separate caches
    # (~/.claude/plugins/marketplaces/ and Claude Desktop's rpm/plugin_*/) — 40 servers
    # the user had never configured. The real project roots are already recorded, so
    # consult exactly those and nothing can smuggle entries in.
    candidates = list(roots) + [d for d in (cj.get("projects") or {})
                                if os.path.isdir(d) and in_roots(d)]
    for d in dict.fromkeys(os.path.abspath(c) for c in candidates):
        if under_excluded(d):
            continue
        p = os.path.join(d, ".mcp.json")
        for name, spec in ((read_json(p, None) or {}).get("mcpServers") or {}).items():
            take(name, spec, p)

    # The desktop app's config is a single global file with no project scope, so
    # --roots cannot meaningfully narrow it.
    desktop = os.path.join(home(), "Library", "Application Support", "Claude",
                           "claude_desktop_config.json")
    for name, spec in ((read_json(desktop, None) or {}).get("mcpServers") or {}).items():
        take(name, spec, "claude_desktop_config.json")

    return found


def unit_mcp(plan, mf, roots):
    found = collect_mcp(roots)
    if not found:
        plan.add("mcp", "skip", "no-op", "-", "no MCP servers defined on the Claude side")
        return
    dest = os.path.join(gemini_config(), "mcp_config.json")
    current = (read_json(dest, None) or {}).get("mcpServers") or {}
    to_add = {}
    for name, (spec, source) in sorted(found.items()):
        if name in current:
            plan.add("mcp", "skip", "name-taken", name,
                     f"name already taken in Antigravity; not overwriting ({source})")
            continue
        entry, warns = translate_mcp(spec)
        if entry is None:
            plan.add("mcp", "warn", "unmigratable", name, "; ".join(warns))
            continue
        for w in warns:
            plan.add("mcp", "warn", "check", name, w)
        to_add[name] = entry
        kind = "remote" if "serverUrl" in entry else "stdio"
        plan.add("mcp", "ok", "add", name, f"{kind} <- {source}")

    if not to_add:
        return

    def fn():
        cur = read_json(dest, None) or {}
        servers = cur.get("mcpServers") or {}
        for name, entry in to_add.items():
            if name in servers:
                continue
            servers[name] = entry
            mf.note_key(dest, "mcpServers", name)
        cur["mcpServers"] = servers
        write_json(dest, cur)

    plan.add("mcp", "ok", "write", dest, f"append {len(to_add)} server(s)", fn)


# --- unit: settings / permissions -------------------------------------------

BASH_RULE = re.compile(r"^Bash\((.+)\)$")
PLAIN_WORD = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]*$")


def translate_permission(rule):
    """Claude `Bash(...)` -> agy `command(<prefix>)`, or None when unmappable.

    Claude's allow-list stores whole command LINES — including quoted prompts and
    escaped parens — not command prefixes. Copying them across verbatim yields
    entries that can never match. So we keep only the leading executable plus one
    plain sub-command word, which is the granularity agy's `command()` matches at.

    This WIDENS the grant (`Bash(git diff:*)` becomes `command(git diff)`), which
    is why the caller never writes these to settings.json without an explicit flag.
    """
    m = BASH_RULE.match(rule.strip())
    if not m:
        return None
    inner = re.sub(r":\*$", "", m.group(1).strip()).strip()
    if not inner or inner.startswith("$"):
        return None
    toks = inner.split()
    if not toks:
        return None
    head = toks[0].rstrip("*")
    # A quote or shell metacharacter in the executable position is not a command.
    if not head or any(ch in head for ch in "|&;<>$`\"'()"):
        return None
    out = head
    if len(toks) > 1 and PLAIN_WORD.match(toks[1]):
        out = f"{head} {toks[1]}"
    return f"command({out})"


def absorb_prefixes(cmds):
    """Drop `command(git diff)` when `command(git)` is already granted."""
    inner = sorted({c[len("command("):-1] for c in cmds})
    keep = [c for c in inner
            if not any(c != k and c.startswith(k + " ") for k in inner)]
    return [f"command({c})" for c in keep]


def unit_settings(plan, mf, apply_permissions):
    allow, seen_files = [], []
    for p in (os.path.join(claude_dir(), "settings.json"),
              os.path.join(claude_dir(), "settings.local.json")):
        d = read_json(p, None)
        if not d:
            continue
        seen_files.append(p)
        allow += ((d.get("permissions") or {}).get("allow") or [])

    mapped, unmapped = set(), []
    for rule in allow:
        t = translate_permission(rule)
        if t:
            mapped.add(t)
        else:
            unmapped.append(rule)

    cur = read_json(agy_settings_path(), None) or {}
    cur_allow = list((cur.get("permissions") or {}).get("allow") or [])
    proposed = absorb_prefixes(mapped)
    new_allow = sorted(k for k in proposed if k not in cur_allow)

    proposal_path = os.path.join(state_dir(), "proposed-permissions.json")
    if new_allow:
        if apply_permissions:
            plan.add("settings", "ok", "permissions", agy_settings_path(),
                     f"{len(allow)} rule(s) -> add {len(new_allow)} command() entries")
        else:
            # Every mapping here widens the grant, so it is never applied implicitly.
            def fn_prop(new_allow=new_allow, proposal_path=proposal_path):
                write_json(proposal_path, {"allow": new_allow})
                mf.files.append(proposal_path)

            plan.add("settings", "ok", "propose-permissions", proposal_path,
                     f"{len(allow)} rule(s) -> propose {len(new_allow)} command() entries only "
                     f"(this widens the grant, so --apply-permissions is required): "
                     f"{', '.join(new_allow[:5])}…", fn_prop)
    if unmapped:
        kinds = sorted({re.split(r"[(_]", u)[0] for u in unmapped})
        plan.add("settings", "warn", "unmappable-permissions", f"{len(unmapped)} rule(s)",
                 f"quoted args, shell metacharacters or non-Bash — no equivalent: {', '.join(kinds[:8])}")

    # trustedWorkspaces is the one setting that maps cleanly.
    cj = read_json(claude_json_path(), None) or {}
    trusted = sorted(p for p, c in (cj.get("projects") or {}).items()
                     if c.get("hasTrustDialogAccepted") and os.path.isdir(p))
    cur_trusted = list(cur.get("trustedWorkspaces") or [])
    add_trusted = [p for p in trusted
                   if not any(p == t or p.startswith(t.rstrip("/") + "/")
                              for t in cur_trusted)]
    if add_trusted:
        plan.add("settings", "ok", "trustedWorkspaces", agy_settings_path(),
                 f"add {len(add_trusted)} trusted path(s)")

    # Report-only: these have no representable equivalent.
    cc = read_json(os.path.join(claude_dir(), "settings.json"), None) or {}
    if cc.get("model"):
        plan.add("settings", "warn", "not-migrated", "model",
                 f"{cc['model']} has no Gemini equivalent; agy's model is left untouched")
    if cc.get("effortLevel"):
        plan.add("settings", "warn", "not-migrated", "effortLevel",
                 f"{cc['effortLevel']} — in agy this is the --effort flag (low|medium|high), not a setting")
    if cc.get("env"):
        plan.add("settings", "warn", "not-migrated", "env",
                 f"{len(cc['env'])} variable(s) — Anthropic/Vertex routing, meaningless to agy")

    write_perms = bool(new_allow) and apply_permissions
    if not (write_perms or add_trusted):
        if not seen_files:
            plan.add("settings", "skip", "no-op", claude_dir(), "no settings found")
        return

    def fn():
        s = read_json(agy_settings_path(), None) or {}
        if write_perms:
            perms = s.get("permissions") or {}
            merged = list(dict.fromkeys(list(perms.get("allow") or []) + new_allow))
            perms["allow"] = merged
            s["permissions"] = perms
            for k in new_allow:
                mf.note_key(agy_settings_path(), "permissions.allow", k)
        if add_trusted:
            tw = list(s.get("trustedWorkspaces") or []) + add_trusted
            s["trustedWorkspaces"] = list(dict.fromkeys(tw))
            for k in add_trusted:
                mf.note_key(agy_settings_path(), "trustedWorkspaces", k)
        write_json(agy_settings_path(), s)

    plan.add("settings", "ok", "write", agy_settings_path(), "append settings", fn)


# --- unit: plugins (native importer via staging HOME) ------------------------

def installed_plugins():
    """Resolve real plugin roots. Modern Claude Code nests them under
    plugins/cache/<marketplace>/<plugin>/<version>/, which the native importer
    cannot see, so we have to enumerate them ourselves."""
    root = os.path.join(claude_dir(), "plugins")
    out = {}
    reg = read_json(os.path.join(root, "installed_plugins.json"), None) or {}
    for pid, entries in (reg.get("plugins") or reg).items():
        if not isinstance(entries, list):
            continue
        for e in entries:
            p = e.get("installPath")
            if p and os.path.isdir(p):
                out[pid.split("@")[0]] = p
    if not out:
        import glob as _glob
        for m in _glob.glob(os.path.join(root, "cache", "*", "*", "*",
                                         ".claude-plugin", "plugin.json")):
            d = os.path.dirname(os.path.dirname(m))
            out.setdefault(os.path.basename(os.path.dirname(d)), d)
    return out


def claude_plugin_mcp(src):
    """Claude declares plugin MCP in .mcp.json or in the manifest's mcpServers."""
    d = read_json(os.path.join(src, ".mcp.json"), None) or {}
    servers = dict(d.get("mcpServers") or {})
    man = read_json(os.path.join(src, ".claude-plugin", "plugin.json"), None) or {}
    for k, v in (man.get("mcpServers") or {}).items():
        servers.setdefault(k, v)
    return servers


def convert_hooks(claude_hooks, plugin):
    """Claude {"hooks":{Event:[...]}} -> Antigravity {"<name>":{Event:[...]}}.

    The native importer copies Claude's shape verbatim; the top-level key happens
    to parse as a hook named "hooks", but the matchers are Claude tool names and
    never match, and three of Claude's events do not exist here at all.
    """
    events = claude_hooks.get("hooks") if "hooks" in claude_hooks else claude_hooks
    out, notes = {}, []
    if not isinstance(events, dict):
        return {}, ["unrecognised hooks.json shape"]
    for cev, groups in events.items():
        aev = HOOK_EVENT_MAP.get(cev)
        if not aev:
            notes.append(f"dropped {cev}: no equivalent event")
            continue
        if not isinstance(groups, list):
            continue
        if aev in HOOK_EVENTS_GROUPED:
            conv = []
            for g in groups:
                matcher = (g or {}).get("matcher", "")
                new_matcher, note = convert_matcher(matcher)
                if note:
                    notes.append(note)
                conv.append({"matcher": new_matcher, "hooks": (g or {}).get("hooks", [])})
            out.setdefault(aev, []).extend(conv)
        else:
            flat = []
            for g in groups:
                flat.extend((g or {}).get("hooks", []) if isinstance(g, dict) and "hooks" in g else [g])
            out.setdefault(aev, []).extend(flat)
    return ({f"{plugin}-hooks": out} if out else {}), notes


def convert_matcher(matcher):
    if not matcher or matcher in ("*", ".*"):
        return "*", None
    names = [n.strip() for n in re.split(r"[|,]", matcher) if n.strip()]
    mapped, unknown = [], []
    for n in names:
        if n in TOOL_MAP:
            mapped.append(TOOL_MAP[n])
        elif n in TOOL_MAP_AMBIGUOUS:
            mapped.append(TOOL_MAP_AMBIGUOUS[n])
            unknown.append(f"{n}->{TOOL_MAP_AMBIGUOUS[n]} (approximate)")
        else:
            unknown.append(f"{n} (unmapped; left as-is)")
            mapped.append(n)
    note = f"matcher {matcher!r}: " + ", ".join(unknown) if unknown else None
    return "|".join(mapped), note


def run_native_import(stage, plugins):
    """Flatten real plugins into a staging HOME and let agy convert them there.
    The importer needs no auth, so nothing touches the real config.

    The plugins are COPIED, not symlinked: the importer does not follow symlinks
    and reports "No claude extensions found" for a tree of links, exit code 0.
    """
    pdir = os.path.join(stage, ".claude", "plugins")
    os.makedirs(pdir, exist_ok=True)
    os.makedirs(os.path.join(stage, ".gemini", "config"), exist_ok=True)
    ignore = shutil.ignore_patterns(".git", ".in_use", "node_modules", "__pycache__")
    for name, src in plugins.items():
        dst = os.path.join(pdir, name)
        if not os.path.exists(dst):
            shutil.copytree(src, dst, symlinks=False, ignore=ignore)
    env = dict(os.environ, HOME=stage)
    env.pop("CLAUDE_CONFIG_DIR", None)
    try:
        r = subprocess.run(["agy", "plugin", "import", "claude"],
                           cwd=stage, env=env, capture_output=True, text=True,
                           timeout=180, stdin=subprocess.DEVNULL)
        return r.returncode, (r.stdout or "") + (r.stderr or "")
    except FileNotFoundError:
        return 127, "agy is not on PATH"
    except subprocess.TimeoutExpired:
        return 124, "agy plugin import claude timed out"


def postprocess_staged(stage, plugins, plan):
    """Repair the native importer's two defects and drop its leftovers."""
    root = os.path.join(stage, ".gemini", "config", "plugins")
    notes = []
    if not os.path.isdir(root):
        return notes
    for name in sorted(os.listdir(root)):
        d = os.path.join(root, name)
        src = plugins.get(name)

        # Defect 1: remote servers arrive as {"command":"", ...} with the URL gone.
        mcp_path = os.path.join(d, "mcp_config.json")
        staged = read_json(mcp_path, None)
        if staged and not src:
            notes.append(f"{name}: staged under a name that does not match any source "
                         f"plugin, so remote MCP entries could NOT be repaired — "
                         f"check {mcp_path} by hand")
        if staged and src:
            original = claude_plugin_mcp(src)
            fixed, restored, dropped = {}, [], []
            for sname, sspec in (staged.get("mcpServers") or {}).items():
                if sspec.get("command"):
                    fixed[sname] = {k: v for k, v in sspec.items()
                                    if v not in ("", None)}
                    continue
                entry, _ = translate_mcp(original.get(sname, {}))
                if entry:
                    fixed[sname] = entry
                    restored.append(sname)
                else:
                    # Removing an entry is not the same as repairing one; say so.
                    dropped.append(sname)
            if restored or dropped:
                write_json(mcp_path, {"mcpServers": fixed})
            if restored:
                notes.append(f"{name}: restored remote MCP ({', '.join(restored)})")
            if dropped:
                notes.append(f"{name}: REMOVED unusable MCP entries the importer "
                             f"emptied and we could not reconstruct "
                             f"({', '.join(dropped)}) — re-add them by hand")

        # Defect 2: Claude's hook schema copied verbatim.
        hpath = os.path.join(d, "hooks.json")
        h = read_json(hpath, None)
        if h:
            conv, hnotes = convert_hooks(h, name)
            if conv:
                write_json(hpath, conv)
            else:
                os.remove(hpath)
            for n in hnotes:
                notes.append(f"{name}: {n}")
        # The hook scripts themselves live in hooks/ and must survive; only the
        # Claude-format manifest inside it is now redundant. Antigravity runs a hook
        # with CWD set to the directory holding hooks.json, so ${CLAUDE_PLUGIN_ROOT}
        # — which it never sets — becomes a plain relative path.
        legacy = os.path.join(d, "hooks", "hooks.json")
        if os.path.isfile(legacy):
            os.remove(legacy)
        if os.path.isfile(hpath):
            txt = read_text(hpath)
            new = txt.replace("${CLAUDE_PLUGIN_ROOT}/", "./") \
                     .replace("$CLAUDE_PLUGIN_ROOT/", "./")
            if new != txt:
                with open(hpath, "w", encoding="utf-8", newline="\n") as fh:
                    fh.write(new)
                notes.append(f"{name}: rewrote ${{CLAUDE_PLUGIN_ROOT}} to a relative path")

        # command-derived SKILL.md loses its name:; the dir name carries it.
        sk = os.path.join(d, "skills")
        if os.path.isdir(sk):
            for s in os.listdir(sk):
                f = os.path.join(sk, s, "SKILL.md")
                if not os.path.isfile(f):
                    continue
                meta, body = split_frontmatter(read_text(f))
                if "name" not in meta:
                    fm = "\n".join(f"{k}: {v}" for k, v in meta.items())
                    with open(f, "w", encoding="utf-8", newline="\n") as fh:
                        fh.write(f"---\nname: {s}\n{fm}\n---\n{body}")

        # hooks/ is NOT junk — it holds the executables the converted hooks.json
        # points at. Only the Claude-side inputs the importer already consumed go.
        for junk in (".claude-plugin", "commands", ".mcp.json"):
            p = os.path.join(d, junk)
            if os.path.isdir(p):
                shutil.rmtree(p)
            elif os.path.isfile(p):
                os.remove(p)
    return notes


def unit_plugins(plan, mf):
    plugins = installed_plugins()
    if not plugins:
        plan.add("plugins", "skip", "no-op", os.path.join(claude_dir(), "plugins"),
                 "no installed plugins")
        return
    dest_root = os.path.join(gemini_config(), "plugins")
    fresh = {n: p for n, p in plugins.items()
             if not os.path.isdir(os.path.join(dest_root, n))}
    for n in sorted(set(plugins) - set(fresh)):
        plan.add("plugins", "skip", "exists", n, "a plugin of that name already exists in Antigravity")

    # The importer keys off .claude-plugin/plugin.json and silently ignores anything
    # without one, so surface those here rather than letting them vanish.
    for n in sorted(fresh):
        if not os.path.isfile(os.path.join(fresh[n], ".claude-plugin", "plugin.json")):
            plan.add("plugins", "warn", "no-manifest", n,
                     "no .claude-plugin/plugin.json, so the native importer skips it")
    fresh = {n: p for n, p in fresh.items()
             if os.path.isfile(os.path.join(p, ".claude-plugin", "plugin.json"))}
    if not fresh:
        return

    def fn():
        stage = tempfile.mkdtemp(prefix="agy-migrate-")
        try:
            rc, out = run_native_import(stage, fresh)
            if rc != 0:
                print(f"    · native importer failed (rc={rc}): "
                      f"{out.strip().splitlines()[-1] if out.strip() else ''}")
                return False
            for n in postprocess_staged(stage, fresh, plan):
                print(f"    · {n}")
            sroot = os.path.join(stage, ".gemini", "config", "plugins")
            staged = sorted(os.listdir(sroot)) if os.path.isdir(sroot) else []
            if not staged:
                # rc==0 with no output is how the importer reports "found nothing".
                # Reporting success here would be a lie.
                print(f"    · native importer produced nothing: "
                      f"{out.strip() or '(no output)'}")
                return False
            copied = 0
            for name in staged:
                target = os.path.join(dest_root, name)
                if os.path.exists(target):
                    continue
                shutil.copytree(os.path.join(sroot, name), target)
                mf.trees.append(target)     # we created it whole; undo removes it whole
                copied += 1
            print(f"    · placed {copied}/{len(fresh)} plugin(s)")
            sman = read_json(os.path.join(stage, ".gemini", "config",
                                          "import_manifest.json"), None)
            if sman:
                dman_path = os.path.join(gemini_config(), "import_manifest.json")
                preexisting = os.path.exists(dman_path)
                dman = read_json(dman_path, None) or {"imports": []}
                have = {i.get("name") for i in dman.get("imports", [])}
                for i in sman.get("imports", []):
                    if i.get("name") not in have:
                        dman.setdefault("imports", []).append(i)
                        mf.note_key(dman_path, "imports", i.get("name"))
                write_json(dman_path, dman)
                if not preexisting:
                    mf.created_json.append(dman_path)
        finally:
            shutil.rmtree(stage, ignore_errors=True)

    plan.add("plugins", "ok", "native-import", ", ".join(sorted(fresh)),
             f"native-import {len(fresh)} plugin(s) via a staging HOME, then repair "
             "remote MCP + hooks and merge", fn)


# --- uninstall ---------------------------------------------------------------

def do_uninstall(apply_):
    mf = Manifest()
    n = 0
    for p in mf.symlinks:
        sibling = os.path.join(os.path.dirname(p), "CLAUDE.md")
        managed_link = os.path.islink(p)
        if not managed_link and os.path.isfile(p) and os.path.isfile(sibling):
            try:
                managed_link = os.path.samefile(p, sibling)
            except OSError:
                managed_link = False
        if managed_link:
            print(f"  remove link    {p}")
            n += 1
            if apply_:
                os.unlink(p)
    for p in mf.files:
        if os.path.isfile(p) and (MARKER in read_text(p) or p.endswith(".json")):
            print(f"  remove file    {p}")
            n += 1
            if apply_:
                os.remove(p)
    for p in mf.trees:
        # Only ours: recorded because we created the whole tree in one copytree.
        if os.path.isdir(p):
            print(f"  remove tree    {p}")
            n += 1
            if apply_:
                shutil.rmtree(p)
    for path, pointers in mf.json_keys.items():
        d = read_json(path, None)
        if not d:
            continue
        for pointer, keys in pointers.items():
            node, parent, last = d, None, None
            for part in ([] if pointer == "" else pointer.split(".")):
                parent, last = node, part
                node = node.get(part) if isinstance(node, dict) else None
                if node is None:
                    break
            if node is None:
                continue
            for k in keys:
                if isinstance(node, dict) and k in node:
                    print(f"  unset {path} :: {pointer}.{k}")
                    n += 1
                    if apply_:
                        del node[k]
                elif isinstance(node, list):
                    before = len(node)
                    # Entries are plain strings (trustedWorkspaces), {"path": …}
                    # (skills.json) or {"name": …} (import_manifest).
                    node[:] = [x for x in node
                               if x != k and not (isinstance(x, dict)
                                                  and k in (x.get("path"), x.get("name")))]
                    if len(node) != before:
                        print(f"  unset {path} :: {pointer}[{k}]")
                        n += 1
        if apply_:
            write_json(path, d)
    for p in mf.created_json:
        d = read_json(p, None)
        if d is None:
            continue
        if isinstance(d, dict) and any(v for v in d.values()):
            print(f"  keep file      {p} (still holds entries we did not add)")
            continue
        print(f"  remove file    {p}")
        n += 1
        if apply_:
            os.remove(p)
    for p in sorted(mf.dirs, key=len, reverse=True):
        if os.path.isdir(p) and not os.listdir(p):
            print(f"  rmdir          {p}")
            n += 1
            if apply_:
                os.rmdir(p)
    print(f"\n{n} item(s) " + ("removed" if apply_ else "would be removed (--apply to do it)"))
    if apply_:
        for f in (manifest_path(),):
            if os.path.exists(f):
                os.remove(f)
    return 0


# --- report ------------------------------------------------------------------

SYM = {"ok": "✓", "warn": "⚠", "skip": "·", "err": "✗"}


def print_report(plan, apply_):
    for unit in UNITS:
        items = plan.by_unit(unit)
        if not items:
            continue
        print(f"\n{C['hdr']}[{unit}]{C['off']}")
        for i in items:
            col = C.get(i["level"], "")
            print(f"  {col}{SYM.get(i['level'],'·')}{C['off']} "
                  f"{i['action']:<20} {i['target']}")
            if i["detail"]:
                print(f"      {i['detail']}")
    c = plan.counts()
    print(f"\n{C['hdr']}Total{C['off']}: {c.get('ok',0)} to apply / {c.get('warn',0)} warning(s) / "
          f"{c.get('skip',0)} skipped")
    if not apply_:
        print(f"\n{C['warn']}This was a dry run. Pass --apply to perform it.{C['off']}")


# --- main --------------------------------------------------------------------

class _Parser(argparse.ArgumentParser):
    """argparse exits 2 on a bad flag; the plugin's shared table calls 2 "agy
    failed" and 1 "usage error". Conform to the table rather than add an exception."""

    def error(self, message):
        self.print_usage(sys.stderr)
        sys.stderr.write(f"{self.prog}: error: {message}\n")
        sys.exit(1)


def main(argv=None):
    ap = _Parser(
        prog="agy-migrate",
        description="Migrate a Claude Code setup (skills, memory, CLAUDE.md, MCP, "
                    "plugins, permissions) to the Antigravity CLI.")
    ap.add_argument("--apply", action="store_true", help="actually write (default is a dry run)")
    ap.add_argument("--only", default="", help=f"limit to these units: {','.join(UNITS)}")
    ap.add_argument("--skip", default="", help="exclude these units")
    ap.add_argument("--include-repos", action="store_true",
                    help="also write inside git repos (AGENTS.md symlink, .agents/rules)")
    ap.add_argument("--include-orphan-memory", action="store_true",
                    help="fold memory whose source directory no longer exists into global rules")
    ap.add_argument("--no-register-projects", action="store_true",
                    help="skip agy project registration (workspace .agents/ then stays inert)")
    ap.add_argument("--roots", default="",
                    help="comma-separated roots to scan for CLAUDE.md / .mcp.json "
                         "(default: the real project paths recorded in ~/.claude.json)")
    ap.add_argument("--apply-permissions", action="store_true",
                    help="actually write the translated allow-list to settings.json. Off by "
                         "default: Claude's allow-list holds whole command lines, so any "
                         "translation widens the grant")
    ap.add_argument("--uninstall", action="store_true", help="remove what a previous run generated")
    ap.add_argument("--json", action="store_true", help="emit a machine-readable plan")
    args = ap.parse_args(argv)

    if args.uninstall:
        return do_uninstall(args.apply)

    # 18: a missing prerequisite, not a usage error and not an agy failure.
    if not os.path.isdir(claude_dir()):
        print(f"{C['err']}No Claude Code config directory at {claude_dir()}{C['off']}")
        return 18
    if not os.path.isdir(gemini_root()):
        print(f"{C['err']}No Antigravity install at {gemini_root()} "
              f"— run agy once first{C['off']}")
        return 18

    only = {u for u in args.only.split(",") if u} or set(UNITS)
    skip = {u for u in args.skip.split(",") if u}
    unknown = sorted((only | skip) - set(UNITS))
    if unknown:
        # Exit 1 = usage error, per the shared table. Silently planning nothing would
        # be indistinguishable from "you have nothing to migrate".
        print(f"{C['err']}unknown unit(s): {', '.join(unknown)}{C['off']}\n"
              f"known units: {', '.join(UNITS)}")
        return 1
    active = [u for u in UNITS if u in only and u not in skip]

    roots = [os.path.abspath(os.path.expanduser(r))
             for r in args.roots.split(",") if r] or default_roots()

    print(f"{C['hdr']}Claude Code{C['off']}  {claude_dir()}")
    print(f"{C['hdr']}Antigravity{C['off']}  {gemini_config()}")
    print(f"{C['hdr']}Scanning{C['off']}     {len(roots)} root(s)"
          f"{'' if args.roots else ' (override with --roots)'}")

    mf = Manifest()
    plan = Plan()
    if "plugins" in active:
        unit_plugins(plan, mf)
    if "skills" in active:
        unit_skills(plan, mf)
    if "claudemd" in active:
        unit_claudemd(plan, mf, roots, args.include_repos)
    if "memory" in active:
        unit_memory(plan, mf, args.include_orphan_memory, args.include_repos,
                    not args.no_register_projects)
    if "mcp" in active:
        unit_mcp(plan, mf, roots)
    if "settings" in active:
        unit_settings(plan, mf, args.apply_permissions)

    if args.json:
        print(json.dumps([{k: v for k, v in i.items() if k != "fn"}
                          for i in plan.items], indent=2, ensure_ascii=False))
        return 0

    print_report(plan, args.apply)
    if not args.apply:
        return 0

    dest = backup([gemini_config(), agy_settings_path()])
    print(f"\nBackup: {dest}")
    failed = []
    try:
        for i in plan.items:
            if i["level"] != "ok" or not i["fn"]:
                continue
            try:
                res = i["fn"]()
            except Exception as e:                       # keep going; report at the end
                res, e_txt = False, f"{type(e).__name__}: {e}"
                print(f"  {C['err']}✗{C['off']} {i['action']} {i['target']} — {e_txt}")
            else:
                mark = C["err"] + "✗" if res is False else C["ok"] + "✓"
                print(f"  {mark}{C['off']} {i['action']} {i['target']}")
            if res is False:
                failed.append(f"{i['unit']}/{i['action']}")
            # Saved per step, not once at the end: a crash halfway through must still
            # leave --uninstall able to find everything written so far.
            mf.save()
    finally:
        mf.runs.append({"at": now_iso(), "units": active, "backup": dest,
                        "failed": failed})
        mf.save()

    if failed:
        print(f"\n{C['err']}{len(failed)} step(s) failed:{C['off']} {', '.join(failed)}")
        print(f"Everything that did succeed is tracked; "
              f"revert with: agy-migrate --uninstall --apply")
        print(f"Backup of the previous state: {dest}")
        # 17, not 2: argparse already exits 2 for a bad flag, and the plugin's shared
        # table calls 2 "agy failed". Neither is what happened here.
        return 17
    print("\nDone. To revert: agy-migrate --uninstall --apply")
    return 0


if __name__ == "__main__":
    sys.exit(main())
