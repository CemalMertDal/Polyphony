...
if on_windows_native; then
  if resolve_bridge_python; then
    ...
    VENDOR_WIN="$(windows_path "$HERE/../vendor/agy-headless-bridge/src")"
    export PYTHONPATH="$VENDOR_WIN${PYTHONPATH:+;$PYTHONPATH}"
    if BRIDGE_DESC="$("${BRIDGE_PY[@]}" -c 'import agy_headless_bridge, winpty; print(agy_headless_bridge.__version__)' 2>/dev/null)"; then
      ok "vendored agy-headless-bridge $BRIDGE_DESC + pywinpty available"
    else
      bad "vendored agy-headless-bridge / pywinpty not importable by ${BRIDGE_PY[*]}"
      info "fix: \`${BRIDGE_PY[*]} -m pip install -U pywinpty\`"
    fi
  else
