...
module only validates one UTF-8 JSON request, invokes the vendored bridge package,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return _fail(
            "PACKAGE_MISSING",
            f"vendored agy-headless-bridge is not importable by {sys.executable}: {exc}",
            EXIT_BRIDGE_MISSING,
        )
