...
- In Claude desktop, the initial AskUserQuestion selection is persisted from the tool's PostToolUse
  result before the next substantive call; a plain text restatement is not required.
...
- Keep inline Agy prompts under **24,000 characters and 3,500 words** (the shared safety budget for Claude Bash and Codex MCP). If a task exceeds either limit, do not retry the same mega-prompt: compress the contract, split independent work across sequential/parallel workers, or pass the prompt through stdin (`agy-delegate ... -`) / a task file. The hook and wrapper enforce this transport guard; stdin/file prompts remain available for intentionally larger inputs.
