...
- Preserved the 0.26.1 Claude Code commands, skill, agent, hooks, wrappers, tier mappings,
  permissions, structured output, timeouts, and exit-code behavior.
- Added a thin Codex plugin, skill, and MCP adapter that call the same wrappers directly.
- Vendored the unchanged `agy-headless-bridge` v1.2.1 source; native Windows now needs
  `pywinpty`, not a separate bridge package installation.
- Packaged the advisory Agy opportunity reminder for Claude Code and Codex.
