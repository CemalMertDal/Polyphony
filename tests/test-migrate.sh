...
if ! python3 -c 'import sys' >/dev/null 2>&1 && command -v py >/dev/null 2>&1; then
  ...
  PY_LAUNCHER="$(py -3 -c 'import sys; print(sys.executable)')"
  case "$(uname -s 2>/dev/null)" in
    MINGW*|MSYS*|CYGWIN*) PY_LAUNCHER="$(cygpath -am "$PY_LAUNCHER")" ;;
  esac
...
H="$TMP/home"
case "$(uname -s 2>/dev/null)" in
  MINGW*|MSYS*|CYGWIN*) H="$(cygpath -am "$H")" ;;
esac
export HOME="$H"
...
p=[os.path.normcase(os.path.abspath(x['path'])) for x in e]
want=os.path.normcase(os.path.abspath(os.path.join('$H','.claude','skills')))
sys.exit(0 if want in p and not any(x.startswith('~') for x in p) else 1)"; then
...
if [ -L "$REPO/AGENTS.md" ] || python3 -c "import os,sys; sys.exit(0 if os.path.samefile('$REPO/AGENTS.md','$REPO/CLAUDE.md') else 1)" 2>/dev/null; then ok "AGENTS.md linked to CLAUDE.md"
