...
out=$(PATH="$TMP/min" "$BASH_BIN" "$CLOUD" --service svc 2>&1); rc=$?
...
python3 - "$HOOKS/policy-context.json" <<'PY' 2>/dev/null; rc=$?
import json,sys
...
err=$( { AGY_PATH= LOCALAPPDATA= PATH="/usr/bin:/bin" "$HOOKS/check-agy.sh" >/dev/null; } 2>&1 ); rc=$?
